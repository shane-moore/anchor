...
use types::{
    AggregateAndProof, Attestation, ContributionAndProof, Domain, EthSpec, Hash256, SelectionProof,
    SignedRoot, Slot,
};
    ...
    AnchorValidatorStore<T, E, C>
{
    ...
    fn resolve_decided_aggregate(
        ...
        // The attestation shape selects the signing-root merkleization. Gloas bytes remain SSZ
        // compatible with Electra, so decoding every post-Electra value as Electra would silently
        // produce the wrong root.
